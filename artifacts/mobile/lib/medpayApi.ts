...
export async function medpayApi<T>(path: string, init?: RequestInit): Promise<T> {
...
export type RegisterAgency = {
  ...
  totalPaid: number; totalRemaining: number; status: "paid" | "partial" | "outstanding";
};

export type MonthlyRegister = {
  month: string;
  summary: { totalAgencies: number; totalBills: number; totalBillAmount: number; totalPaid: number; totalRemaining: number };

  ...
  const handleSave = async () => {
    ...
    try {
      ...
      if (selectedCustomerId) {
        queryClient.invalidateQueries({ queryKey: getGetCustomerInvoicesQueryKey(parseInt(selectedCustomerId)) });
      }
      router.back();
    } catch {
    ...
    <View style={[styles.container, { backgroundColor: colors.background }]}>
      <KeyboardAwareScrollViewCompat contentContainerStyle={styles.content} keyboardShouldPersistTaps="handled">
        ...
        <View style={styles.field}>
          ...
          <TextInput value={invoiceNumber} onChangeText={setInvoiceNumber} placeholder="e.g. INV-2024-001" placeholderTextColor={colors.mutedForeground} style={inputStyle} />
        </View>

        <View style={styles.field}>
          <Text style={[styles.label, { color: colors.mutedForeground }]}>Invoice Date *</Text>
          <TextInput value={invoiceDate} onChangeText={setInvoiceDate} placeholder="YYYY-MM-DD" placeholderTextColor={colors.mutedForeground} style={inputStyle} />
        </View>
        ...
        <View style={styles.field}>
          ...
          {dueDate && <Text style={[styles.dueText, { color: colors.mutedForeground }]}>Due date: {dueDate}</Text>}
        </View>
...
const styles = StyleSheet.create({
  ...
  dueText: { fontSize: 11, fontFamily: "Inter_400Regular" },

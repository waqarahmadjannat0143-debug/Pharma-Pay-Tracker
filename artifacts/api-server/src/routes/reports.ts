...
router.get("/outstanding", async (req: AuthRequest, res) => {
  try {
    const rows = await db.select({
      customerId: customersTable.id, customerName: customersTable.name, mobile: customersTable.mobile,
    ...
    }).from(customersTable).leftJoin(invoicesTable, eq(invoicesTable.customerId, customersTable.id)).groupBy(customersTable.id)
  ...
  } catch (err) { req.log?.error({ err }, "Failed outstanding"); res.status(500).json({ error: "Failed to fetch report" }); }
});
...
router.get("/invoice-payments/:invoiceId", async (req: AuthRequest, res) => {
  ...
  } catch (err) { req.log?.error({ err }, "Failed invoice payment history"); res.status(500).json({ error: "Failed to fetch invoice payment history" }); }
});

router.get("/payment-history", async (req: AuthRequest, res) => {
  try {
    const { customerId, fromDate, toDate } = req.query; const conditions: any[] = [];
    if (customerId) conditions.push(eq(paymentsTable.customerId, parseInt(customerId as string)));
    if (fromDate) conditions.push(gte(paymentsTable.paymentDate, fromDate as string)); if (toDate) conditions.push(lte(paymentsTable.paymentDate, toDate as string));
  ...
  } catch (err) { req.log?.error({ err }, "Failed payment history"); res.status(500).json({ error: "Failed to fetch report" }); }
});

router.get("/date-wise-collection", async (req: AuthRequest, res) => {
  try { const { fromDate, toDate } = req.query; const rows = await db.execute(sql`SELECT payment_date AS date, SUM(amount::numeric) AS amount, COUNT(*) AS count FROM payments WHERE payment_date >= ${fromDate || '2000-01-01'} AND payment_date <= ${toDate || '2099-12-31'} GROUP BY payment_date ORDER BY payment_date DESC`); res.json((rows.rows as any[]).map(r => ({ date: r.date, amount: Number(r.amount), count: Number(r.count) }))); }
  catch (err) { req.log?.error({ err }, "Failed collection"); res.status(500).json({ error: "Failed to fetch report" }); }
});

router.get("/monthly-collection", async (req: AuthRequest, res) => {
  try { const year = req.query.year ? parseInt(req.query.year as string) : new Date().getFullYear(); const rows = await db.execute(sql`SELECT EXTRACT(MONTH FROM payment_date::date) AS month, EXTRACT(YEAR FROM payment_date::date) AS year, SUM(amount::numeric) AS amount FROM payments WHERE EXTRACT(YEAR FROM payment_date::date)=${year} GROUP BY month,year ORDER BY month`); const months=["Jan","Feb","Mar","Apr","May","Jun","Jul","Aug","Sep","Oct","Nov","Dec"]; res.json((rows.rows as any[]).map(r => ({ year:Number(r.year), month:Number(r.month), amount:Number(r.amount), label:months[Number(r.month)-1] }))); }
  catch (err) { req.log?.error({ err }, "Failed monthly"); res.status(500).json({ error: "Failed to fetch report" }); }
});
